...
同时确认已在“系统设置 → 键盘 → 文本输入 → 编辑”中添加“简体拼音”。此功能直接选择简体拼音，不依赖 `Control–空格` 快捷键。

## 工作方式

```text
F13 → Amatsume init → 系统输入源 API → 简体拼音

Finder 空白处右键 → Finder Sync 扩展 → Amatsume init → 默认终端 + 当前目录
```

本应用将 F13 固定映射为简体拼音，不模拟其他 Fn/地球仪组合键。
